...
export function deployProgram(payerKeyFile: string): web3.PublicKey {
    ...
    const keypair = web3.Keypair.fromSecretKey(Uint8Array.from(JSON.parse(bytes)));
    execSync([
        "solana program deploy", agnosticOrderbookSo,
        "--program-id", keyfile,
        "-u localhost",
        "-k", payerKeyFile,
...
export async function airdropPayer(connection: web3.Connection, key: web3.PublicKey) {
    while (true) {
        try {
            const signature = await connection.requestAirdrop(key, 100 * web3.LAMPORTS_PER_SOL);
