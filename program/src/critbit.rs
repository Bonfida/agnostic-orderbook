...
use bytemuck::{cast, cast_mut, cast_ref, cast_slice, cast_slice_mut};
...
// A Slab account contains the data for a slab header and an array of nodes of a critbit tree
...
impl LeafNode {
    #[inline]
    pub fn new(owner_slot: u8, key: u128, owner: Pubkey, quantity: u64) -> Self {
        LeafNode {
...
impl<'a> Slab<'a> {
    #[inline]
    pub fn new(bytes: &'a mut [u8]) -> &mut Self {
        ...
        let slab: &mut Self = bytes;
        slab.check_size_align(); // check alignment
        slab
    }

    #[inline]
    pub fn assert_minimum_capacity(&self, capacity: u32) -> AOResult {
        if self.nodes().len() <= (capacity as usize) * 2 {
            Err(AOError::SlabTooSmall)?
        }
        Ok(())
    }
